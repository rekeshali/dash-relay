...
def test_registry_dispatches_registered_action():
    app = _make_app()
...
def test_registry_ignores_unknown_action():
...
def test_registry_passes_full_event_to_handler():
    app = _make_app()
    events = relay.registry(app, state="state")
    seen = {}
...
def test_registry_registers_exactly_one_callback():
    app = _make_app()
    before = len(app.callback_map)
    relay.registry(app, state="state")
...
def test_registry_multi_state_dispatches_and_returns_tuple():
...
def test_registry_multi_state_supports_explicit_tuple_return():
...
def test_registry_multi_state_registers_exactly_one_callback():
    app = _make_multi_state_app()
    before = len(app.callback_map)
    relay.registry(app, state=["a", "b"])
    assert len(app.callback_map) == before + 1


def test_registry_rejects_empty_state_list():
    app = _make_app()
    try:
        relay.registry(app, state=[])
    except ValueError:
        pass
    else:
